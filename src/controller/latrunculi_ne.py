...
class Latrunculi_ne(Game):
    ...
    def actions(self, state):
        ...
        if state.player:
            ...
            enemy_captured = 2
        actionsList = set() #we might have to add a pass option???
        ...
        while not it.finished:
            ...
            if it[0] == current_player: #If the current piece on the board is owned by the current player
                ...
                actionsList.add(Action((i, j), (i, j))) # action to remove an opponents captured piece
            it.iternext()

        # if actionsList == []:
        #     actionsList.add(None)
        
        return list(actionsList)
        ...
        if (i + direction) >= 0 and (i + direction) < self.size: # check for whether 1 square NORTH/SOUTH is within the board
            if board[i + direction][j] == 0: #check for NORTH/SOUTH square being empty
                if (j - 1) >= 0 and (j + 1) < self.size: #check for whether insta-capture is possible or if we are too close to edge of the board
                    self.check_for_capture_and_suicide_west_or_east_of_given_piece(actionsList, i, j, (i+direction), j, player, board)
                else:  #if there is no chance for insta capture, create action to empty square
                    actionsList.add(Action((i, j), (i+direction, j)))
                    ...
                    if x >= 0 and x < self.size: #check that x squares north/south is within the bounds of the board 
                        if board[x + (-1*direction)][j] != 0: #check if there is a piece on the odd number square north/south... #this is a double check for the first jump, might want to optimize it...
                            if board[x][j] == 0: #checks for the even number square north/south being empty, if the square before was occupied
                                ...
                                self.check_for_capture_and_suicide_all_directions_of_given_piece(actionsList, i, j, x, j, player, board)
                            else:
        ...
        if (j + direction) >= 0 and (j + direction) < self.size: # check for whether 1 square WEST/EAST is within the board
            if board[i][j + direction] == 0: #check for WEST/EAST square being empty
                if (i - 1) >= 0 and (i + 1) < self.size: #check for whether insta-capture is possible or if we are too close to edge of the board
                    self.check_for_capture_and_suicide_north_or_south_of_given_piece(actionsList, i, j, i, (j+direction), player, board)
                else:  #if there is no chance for insta capture, create action to empty square
                    actionsList.add(Action((i, j), (i, j+direction)))
                    ...
                    if x >= 0 and x < self.size: #check that x squares WEST/EAST is within the bounds of the board
                        if board[i][(x + (-1*direction))] != 0: #check if there is a piece on the odd number square WEST/EAST... #this is a double check for the first jump, might want to optimize it...
                            if board[i][x] == 0: #checks for the even number square WEST/EAST being empty, if the square before was occupied
                                #check for capture/suicide in all directions of the jump destination
                                self.check_for_capture_and_suicide_all_directions_of_given_piece(actionsList, i, j, i, x, player, board)
                            else:
                                break #jump chain is broken
                        else:
                            break #jump chain is broken
    ...
    def check_for_capture_and_suicide_all_directions_of_given_piece(self, actionsList, iOrigin, jOrigin, iDest, jDest, player, board):
        self.check_for_capture_and_suicide_west_or_east_of_given_piece(actionsList, iOrigin, jOrigin, iDest, jDest, player, board) #check for action WEST/EAST
        self.check_for_capture_and_suicide_north_or_south_of_given_piece(actionsList, iOrigin, jOrigin, iDest, jDest, player, board) #check for action NORTH/SOUTH
        
    def check_for_capture_and_suicide_west_or_east_of_given_piece(self, actionsList, iOrigin, jOrigin, iDest, jDest, player, board):
        ...
        if (jDest - 1) >= 0 and (jDest + 1) < self.size: #check if there is room for a possible insta-capture WEST/EAST
            if board[iDest][jDest + 1] == enemy_player and board[iDest][jDest - 1] == enemy_player: #check for insta capture
                if (jDest - 2) >= 0 and board[iDest][jDest - 2] == player and (jDest - 2) != jOrigin: #check for possible suicide action to the west
                        actionsList.add(Action((iOrigin, jOrigin), (iDest, jDest)))
    ...
    def check_for_capture_and_suicide_north_or_south_of_given_piece(self, actionsList, iOrigin, jOrigin, iDest, jDest, player, board):
        enemy_player = -1*player
